...
1. [x] `os.environ` reads in deep core modules are reduced to near-zero (allow list: config module + composition root).
2. [x] Commands build an `AppContext` from config and pass explicit values (no hidden defaults).
3. [x] Tests can instantiate services without mutating global env.
4. [x] `make ci` passes.
