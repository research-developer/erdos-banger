...
1. [x] A single SSOT exists for loop contracts:
   - Spec-012 updated with "Implementation Deviations" section documenting code as SSOT.
2. [x] `run_loop` is reduced to ≤ ~120 LOC or replaced by a small coordinator calling well-named helpers/classes.
   - `run_loop` is now 58 LOC in `src/erdos/core/loop/runner.py`.
3. [x] Public imports remain stable (re-export shim allowed):
   - `from erdos.core.loop import run_loop, LoopStatus, LoopResult` continues to work via `__init__.py`.
4. [x] Tests cover loop contract semantics explicitly:
   - `test_json_llm_required_returns_failure`, `test_no_apply_mode_with_path`, `test_applies_patch_and_checks`.
5. [x] `make ci` passes (ruff, mypy, pytest, coverage).
