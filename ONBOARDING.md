...
**Where we are today:** You can run this loop manually with `erdos ask` + `erdos lean check`, or automate iterations with `erdos loop run`.
...
- **Problem dataset harness**:
  ...
  - runtime input: an **enriched** dataset (titles/statements/references) provided via `ERDOS_DATA_PATH` or created/updated by `erdos sync website` (default: `data/problems_enriched.yaml`, gitignored)
...
- **Not a general-purpose paper discovery engine** - it includes optional research API integrations (Exa / Semantic Scholar / zbMATH) but is optimized for problem-centric workflows
- **Not a web app** - CLI-first, designed for automation and AI agents
- **Not storing paywalled PDFs** - only metadata + open-access content (arXiv source). PDF conversion exists as an opt-in extra (`uv sync --extra pdf`).
...
Notes:
- `data/erdosproblems/` is upstream **metadata-only**; `erdos sync website` produces/updates a local enriched dataset at `data/problems_enriched.yaml` (gitignored).
- `erdos loop run` automates the LLM→Lean iteration; you can also run the loop manually with `erdos ask` + `erdos lean check`.
- OpenAlex and other providers are integrated via the `MetadataProvider` abstraction (see `docs/_archive/specs/spec-022-metadata-provider-orchestration.md`).
...
This philosophy is supported by structured run logs and a filesystem research workspace (`erdos logs`, `erdos research …`, `erdos dashboard`).
...
|-----------|--------|-------|
| CLI scaffold | ✅ Done | `list`, `show`, `refs`, `search`, `ingest`, `ask`, `lean`, `loop`, `research`, `sync`, `dashboard`, `convert` |
| Problem loader | ✅ Done | Built-in sample dataset by default; supports local enriched dataset overrides via `ERDOS_DATA_PATH` |
| Sync (unified data) | ✅ Done | Pulls from multiple sources and updates local dataset (`erdos sync all`) |
| Ingest (metadata + caches) | ✅ Done | arXiv/Crossref/OpenAlex via provider abstraction |
| Search (FTS5) | ✅ Done | On-disk SQLite index + hybrid search extensions |
| Ask (RAG Q&A) | ✅ Done | Citation-grounded prompt + optional external LLM subprocess |
| Loop (iterative proofs) | ✅ Done | Iterative Lean patch loop + run logs |
| Research workspace | ✅ Done | Filesystem SSOT for leads/attempts/hypotheses/tasks + synthesis |
| Dashboard | ✅ Done | Local progress dashboard |
| MCP server | ✅ Done | Optional tool server integration |
...
[Paul Erdős](https://en.wikipedia.org/wiki/Paul_Erd%C5%91s) was a legendary mathematician who posed thousands of problems, many with cash prizes. The upstream dataset (https://github.com/teorth/erdosproblems) catalogs **over 1,000** problems and evolves over time.
